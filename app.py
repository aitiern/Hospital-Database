# app.py
"""
HealthDB Clinical Platform - Main Landing Page

This is the main entry point for the HealthDB EHR + Analytics app.
It shows high-level KPIs, navigation help, and system architecture
overview. Other functionality lives in the /pages directory.
"""
...
# ---------- Sidebar: provider "login" ----------

def renderLoginSidebar() -> None:
    """
    Simple provider 'login': pick an active provider from a dropdown.
    Stores provider_id and display name in session_state to be used
    by other pages (Provider Dashboard, New Clinical Entries, etc.).
    """
    with st.sidebar:
...
def main() -> None:
    ...
    st.info(
        ...
        "modules. The provider you select in the sidebar will be used by the "
        "Provider Dashboard and New Clinical Entries pages."
    )
    ...
    with arch_col1:
        ...
        st.markdown(
            ...
            - **Core tables**: patients, providers, facilities, encounters,
              appointments, conditions, medications, treatmentplans,
              treatmentplanitems  
    ...
    st.caption(
        "Built with MariaDB/MariaDB, SQLAlchemy, and Streamlit as part of an "
